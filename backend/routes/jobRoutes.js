...
router.post("/create",auth,async(req,res)=>{
    try{
        ...
        await sendEmail({
            to: user.email,
            subject: "Hire Hub job posted successfully",
            text: `Your job post "${job.title}" at ${job.company} is now live on Hire Hub.`,
            html: `
                <h2>Job posted successfully</h2>
                <p>Your job post <strong>${job.title}</strong> at <strong>${job.company}</strong> is now live.</p>
